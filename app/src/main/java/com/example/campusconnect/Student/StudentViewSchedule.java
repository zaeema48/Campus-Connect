...
public class StudentViewSchedule extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        fetchView();
        ProgressDialog progressDialog= new ProgressDialog(StudentViewSchedule.this);
        progressDialog.setTitle("Getting Schedule...");
        progressDialog.show();

        List<ScheduleModel> schedules=new ArrayList<>();

        StudentAPI.getStudentApiInterface().viewCLassSchedule(publicStudent.getStudentId()).enqueue(new Callback<List<ScheduleModel>>() {
            @Override
            ...
            public void onFailure(Call<List<ScheduleModel>> call, Throwable t) {
                progressDialog.dismiss();
                Toast.makeText(StudentViewSchedule.this, "Error!!", Toast.LENGTH_SHORT).show();
            }
        });
